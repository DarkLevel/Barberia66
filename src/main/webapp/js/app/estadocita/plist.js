...
moduleEstadocita.controller('estadocitaPlistController',  ['$scope', '$http', '$location', 'toolService', '$routeParams', '$anchorScroll',
    function ($scope, $http, $location, toolService, $routeParams, $anchorScroll) {
        ...
        $scope.resetOrder = function () {
            $location.url(`estadocita/plist/` + $scope.rpp + `/` + $scope.page);
        };
        ...
        $scope.ordenar = function (order, align) {
            ...
            $location.url(`estadocita/plist/` + $scope.rpp + `/` + $scope.page + `/` + $scope.orderURLCliente);
        };

        $http({
            method: 'GET',
            url: 'http://localhost:8081/barberia66/barberia66?ob=estadocita&op=getcount'
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/barberia66/barberia66?ob=estadocita&op=getpage&rpp=' + $scope.rpp + '&page=' + $scope.page + $scope.orderURLServidor
        }).then(function (response) {
            $scope.status = response.status;
        ...
        $scope.crear = function () {
            $location.url('estadocita/create');
        };
