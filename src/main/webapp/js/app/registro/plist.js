...
moduleRegistro.controller('registroPlistController',  ['$scope', '$http', '$location', 'toolService', '$routeParams', '$anchorScroll',
    function ($scope, $http, $location, toolService, $routeParams, $anchorScroll) {
        ...
        $scope.resetOrder = function () {
            $location.url(`registro/plist/` + $scope.rpp + `/` + $scope.page);
        };
        ...
        $scope.ordenar = function (order, align) {
            ...
            $location.url(`registro/plist/` + $scope.rpp + `/` + $scope.page + `/` + $scope.orderURLCliente);
        };

        $http({
            method: 'GET',
            url: 'http://localhost:8081/barberia66/barberia66?ob=registro&op=getcount'
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/barberia66/barberia66?ob=registro&op=getpage&rpp=' + $scope.rpp + '&page=' + $scope.page + $scope.orderURLServidor
        }).then(function (response) {
            $scope.status = response.status;
        ...
        $scope.crearPDF = function (id) {
            ...
            for (var i = 0; i < length; i++) {
                if ($scope.ajaxData[i].id === id) {
                    usuario = $scope.ajaxData[i].obj_usuario.apellido1 + ' ' + $scope.ajaxData[i].obj_usuario.apellido2 + ', ' + $scope.ajaxData[i].obj_usuario.nombre;
            ...
            $http({
                method: 'GET',
                url: 'http://localhost:8081/barberia66/barberia66?ob=linea&op=getpageX&rpp=10000&page=1&id_fact=' + id
            }).then(function (response) {
                ...
                for (var x = 0; x <= lineasTotales - 1; x++) {
                    ...
                    doc.text(12, linea, $scope.ajaxLineasFactura[x].obj_producto.nombre);
                    var descripcion = $scope.ajaxLineasFactura[x].obj_producto.descripcion;
                    ...
                    var precioProducto = ($scope.ajaxLineasFactura[x].obj_producto.precio * $scope.ajaxLineasFactura[x].cantidad).toFixed(2).toString();
                    doc.text(170, linea, (precioProducto + ' (' + $scope.ajaxLineasFactura[x].obj_producto.iva + '%)').toString());
                    linea = linea + 13;
                    precio = (precio + ($scope.ajaxLineasFactura[x].obj_producto.precio * $scope.ajaxLineasFactura[x].cantidad));
                    iva = (iva + ($scope.ajaxLineasFactura[x].obj_producto.precio * $scope.ajaxLineasFactura[x].cantidad * ($scope.ajaxLineasFactura[x].obj_producto.iva / 100)));
                ...
                doc.text(115, 285, iva.toFixed(2).toString());
                doc.text(158, 285, (precio + iva).toFixed(2).toString());

                doc.output('save', 'Factura-' + id + '_' + fecha.replace(/\//g, '-') + '.pdf');
            }, function (response) {
                $scope.status = response.status;
