...
moduleTipocita.controller('tipocitaPlistController',  ['$scope', '$http', '$location', 'toolService', '$routeParams', '$anchorScroll',
    function ($scope, $http, $location, toolService, $routeParams, $anchorScroll) {
        ...
        $scope.resetOrder = function () {
            $location.url(`tipocita/plist/` + $scope.rpp + `/` + $scope.page);
        };
        ...
        $scope.ordenar = function (order, align) {
            ...
            $location.url(`tipocita/plist/` + $scope.rpp + `/` + $scope.page + `/` + $scope.orderURLCliente);
        };

        $http({
            method: 'GET',
            url: 'http://localhost:8081/barberia66/barberia66?ob=tipocita&op=getcount'
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/barberia66/barberia66?ob=tipocita&op=getpage&rpp=' + $scope.rpp + '&page=' + $scope.page + $scope.orderURLServidor
        }).then(function (response) {
            $scope.status = response.status;
